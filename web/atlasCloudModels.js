...
const DEFAULT_PROVIDER = "atlascloud";
// WaveSpeed's catalogue needs the API key. It travels in a header, not in the
// query string, so it never reaches the server's access log; see
// atlascloud_routes.py.
const API_KEY_HEADER = "X-MDPack-Api-Key";

function providerOf(node) {
    return widgetOf(node, "provider")?.value || DEFAULT_PROVIDER;
}

function apiKeyOf(node) {
    return widgetOf(node, "api_key")?.value || "";
}

function keyHeaders(apiKey) {
    return apiKey ? { headers: { [API_KEY_HEADER]: apiKey } } : undefined;
}

// One catalogue request per (provider, output, input) triple, shared by every
// node. The provider belongs in the key: the two catalogues are disjoint and
// list entirely different model ids.
const catalogueCache = new Map();

async function fetchModels(outputKind, inputKind, provider = DEFAULT_PROVIDER, apiKey = "") {
    const key = `${provider}|${outputKind}|${inputKind}`;
    ...
    // Only named when it is not the default, so the URLs (and therefore the
    // request the ComfyUI server sees) stay exactly as they were for Atlas
    // Cloud; the route defaults to the same provider.
    if (provider !== DEFAULT_PROVIDER) query.set("provider", provider);
    const request = api
        .fetchApi(`/mdpack/atlas/models?${query.toString()}`, keyHeaders(apiKey))
...
async function refreshModelOptions(node, { keepValue = true } = {}) {
    ...
    const entries = await fetchModels(outputKind, inputKind, providerOf(node), apiKeyOf(node));
...
// One schema request per (provider, model id), shared by every node that
// selects it -- the same vendor prefixes exist on both providers, so the id
// alone would not identify a schema.
const schemaCache = new Map();

async function fetchSchema(modelIdentifier, provider = DEFAULT_PROVIDER, apiKey = "") {
    const cacheKey = `${provider}|${modelIdentifier}`;
    if (schemaCache.has(cacheKey)) return schemaCache.get(cacheKey);
    const providerQuery =
        provider === DEFAULT_PROVIDER ? "" : `&provider=${encodeURIComponent(provider)}`;
    const request = api
        .fetchApi(
            `/mdpack/atlas/schema?model=${encodeURIComponent(modelIdentifier)}${providerQuery}`,
            keyHeaders(apiKey),
        )
        .then((response) => response.json())
        ...
        .then((schema) => {
            ...
            if (schema === null) schemaCache.delete(cacheKey);
            return schema;
        });
    schemaCache.set(cacheKey, request);
    return request;
}
...
// Mirrors IMAGE_COUNT_FIELD_NAMES in atlascloud_models.py: the node's
// num_images widget is routed to whichever of these the selected model
// declares, so a param_n / param_max_images widget supersedes it just as a
// param_num_images one does -- showing both would let the user set a value
// that then loses to params_json without any visible reason.
const IMAGE_COUNT_FIELD_NAMES = ["num_images", "n", "max_images", "num_outputs"];
...
// the image slots.
// A model the catalogue does not know (or a failed schema lookup) answers
// with this empty shape; the node then falls back to sending its own
// conventional fields, so every legacy widget still reaches the request and
// none of them may be hidden. Mirrors the emptiness test resolve_model_schema
// uses in atlascloud_api.py.
function schemaIsUnknown(schema) {
    return (
        !schema ||
        !(
            schema.prompt_field ||
            (schema.fields || []).length ||
            (schema.image_fields || []).length
        )
    );
}
...
function hideSupersededLegacyWidgets(node, schema) {
    ...
    // Same idea for num_images: `n` and `max_images` are the other names the
    // catalogue uses for it, and Python routes the widget's value there.
    const hasCountField = IMAGE_COUNT_FIELD_NAMES.some((name) => declaredFieldNames.has(name));
    const schemaUnknown = schemaIsUnknown(schema);
    for (const legacyName of LEGACY_WIDGET_NAMES) {
        ...
        // width/height cannot be matched by name: build_payload turns them
        // into whichever dimension field the model actually declares -- the
        // "WIDTH*HEIGHT" `size` string, or separate width/height keys. Both
        // of those arrive as their own param_ widgets, and a model that
        // declares neither (754 of WaveSpeed's 953, e.g. anything sized by
        // aspect_ratio/resolution alone) has nowhere to put them at all: the
        // request drops the values with a console line. Either way the pair
        // is dead as soon as a real schema is known, which is exactly when
        // it used to stay on screen inviting a resolution that never
        // applied.
        legacyWidget.hidden = !schemaUnknown && (
            isDimension
            || declaredFieldNames.has(legacyName)
            || (legacyName === "num_images" && hasCountField)
        );
    }
}
...
// Widgets that exist because the node declares them, in declaration order --
// as opposed to the param_* widgets buildParamWidgets appends afterwards.
// ComfyUI serialises widgets_values in exactly this order: required inputs
// first, then the optional ones (reference_urls), then the param_* widgets.
function declaredWidgetsOf(node) {
    return (node.widgets || []).filter((widget) => !widget.name.startsWith(PARAM_PREFIX));
}

// ComfyUI's frontend adds this companion widget to a seed widget by itself;
// INPUT_TYPES never lists it, but it does occupy a widgets_values slot.
const CONTROL_WIDGET_NAME = "control_after_generate";

// How many of the node's widgets come from `required`. `provider` used to be
// the last of those, which is the position the migration below has to know --
// counting all widgets instead would put it one slot too far, right where the
// optional reference_urls sits.
function requiredWidgetCountOf(node, requiredNames) {
    const required = new Set(requiredNames || []);
    if (!required.size) return declaredWidgetsOf(node).length;
    return declaredWidgetsOf(node).filter(
        (widget) => required.has(widget.name) || widget.name === CONTROL_WIDGET_NAME
    ).length;
}

// `provider` decides what every other widget means, so it is declared first.
// It was not always: the first released version appended it last (so that
// workflows saved before it existed kept loading), and before that it did not
// exist at all. widgets_values is a positional array, so a workflow saved
// under either older layout would now restore every value one widget too
// early -- the api key landing in `provider`, the model id in `api_key`, and
// so on down the node.
//
// This runs on load, after ComfyUI has applied widgets_values positionally,
// and re-applies them under the layout the array was actually written with.
// Three layouts have to be told apart, and the first widget's value is what
// distinguishes them: only the current one stores a provider id there.
function migrateWidgetValues(node, info, requiredNames) {
    const storedValues = info?.widgets_values;
    if (!Array.isArray(storedValues) || storedValues.length === 0) return false;
    const providerWidget = widgetOf(node, "provider");
    if (!providerWidget) return false;
    const providerValues = new Set(providerWidget.options?.values || []);
    if (providerValues.has(storedValues[0])) return false; // already provider-first

    const requiredCount = requiredWidgetCountOf(node, requiredNames);
    if (!requiredCount) return false;
    // In the "provider last" layout it sat at the end of the required
    // widgets; in the oldest layout there was no provider value at all and
    // the required block was one shorter. Either way the values to keep
    // start at 0 and run to requiredCount - 1.
    const storedProvider = storedValues[requiredCount - 1];
    const hadProviderLast = providerValues.has(storedProvider);
    const provider = hadProviderLast ? storedProvider : DEFAULT_PROVIDER;
    // The optional widgets that follow -- reference_urls above all -- shift by
    // exactly the same one position. Repairing only the required block would
    // leave reference_urls holding whatever ComfyUI's positional restore put
    // there, which is how a stray "wavespeed" once reached Seedream as a
    // seventh reference image.
    const migrated = [
        provider,
        ...storedValues.slice(0, requiredCount - 1),
        ...storedValues.slice(hadProviderLast ? requiredCount : requiredCount - 1),
    ];

    // param_* widgets are rebuilt from params_json (itself one of the values
    // restored above), so they are skipped rather than fed a shifted value.
    const targets = declaredWidgetsOf(node);
    for (let index = 0; index < targets.length && index < migrated.length; index += 1) {
        // A value the older layout never stored (a widget added since) is left
        // at its default rather than overwritten with undefined.
        if (migrated[index] !== undefined) targets[index].value = migrated[index];
    }
    return true;
}
...
async function applySchema(node) {
    ...
    const schema = await fetchSchema(modelIdentifier, providerOf(node), apiKeyOf(node));
    ...
    async beforeRegisterNodeDef(nodeType, nodeData) {
        ...
        nodeType.prototype.onNodeCreated = function () {
            ...
            if (!filterCallbacksWrapped.has(node)) {
                ...
                // Switching provider replaces the catalogue wholesale, so the
                // stored model id almost certainly does not exist on the new
                // one -- pick the new provider's first entry (keepValue: false)
                // rather than leave a dead id selected.
                for (const filterName of ["output_type", "input_type", "provider"]) {
                ...
                // WaveSpeed's catalogue is only readable with a key, so a node
                // created before the key was pasted has an empty dropdown.
                // Pasting the key has to trigger the fetch that could not
                // succeed earlier; the stored model id is kept (keepValue) so
                // a saved workflow does not lose its selection.
                const apiKeyWidget = widgetOf(node, "api_key");
                if (apiKeyWidget) {
                    const previousKeyCallback = apiKeyWidget.callback;
                    apiKeyWidget.callback = function (value) {
                        const callbackResult = previousKeyCallback?.apply(this, arguments);
                        refreshModelOptions(node, { keepValue: true });
                        applySchema(node);
                        return callbackResult;
                    };
                }
        ...
        // The declared order of the required inputs, captured at registration:
        // migrateWidgetValues needs to know where the required block ends and
        // the optional widgets (reference_urls) begin.
        const requiredInputNames = Object.keys(nodeData.input?.required || {});

        const onConfigure = nodeType.prototype.onConfigure;
        nodeType.prototype.onConfigure = function (info) {
            const result = onConfigure?.apply(this, arguments);
            // Runs before the refreshes below, which read model/provider.
            migrateWidgetValues(this, info, requiredInputNames);
